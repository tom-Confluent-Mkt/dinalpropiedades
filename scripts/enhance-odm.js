...
const CACHE_FILE = join(DATA_DIR, 'desc-cache-odm.json');
...
async function run() {
  ...
  console.log(`[${start.toISOString()}] Starting ODM description enhancement… ${FORCE ? '(--force)' : ''}`);

  const propsFile = join(DATA_DIR, 'odm-properties.json');
  const devsFile  = join(DATA_DIR, 'odm-developments.json');
  ...
  await processCollection(propsData.objects, cache, 'odm-properties');
  await processCollection(devsData.objects,  cache, 'odm-developments');

  writeFileSync(CACHE_FILE,  JSON.stringify(cache,     null, 2));
  writeFileSync(propsFile,   JSON.stringify(propsData, null, 2));
  writeFileSync(devsFile,    JSON.stringify(devsData,  null, 2));

  console.log(`[${new Date().toISOString()}] ODM Enhancement complete.`);
}
