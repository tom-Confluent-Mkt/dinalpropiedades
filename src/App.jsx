  ...
  return (
    <Link to={`/propiedad/${propSlug(prop)}`} className={`${className} bg-white rounded-[1rem] p-4 shadow-sm border border-primary/10 hover:shadow-xl hover:-translate-y-2 hover:border-accent transition-all duration-300 group cursor-pointer relative flex flex-col h-full block`}>
      ...
      <div className="px-2 flex-grow flex flex-col">
        ...
        <p className="font-heading text-sm font-semibold text-primary/80 mb-6 flex items-center gap-2"><MapPin size={16} className="text-accent shrink-0"/> {prop.address ? `${prop.address}${prop.location?.name ? `, ${prop.location.name}` : ''}` : prop.location?.name}</p>
...
const STATIC_TERMINADOS = [
  {
    ...
    sustainability: ['Doble muro de ladrillo con cámara de aire', 'Aberturas de doble vidrio (DVH)'],
  ...
  },
  {
    ...
    specs: '9 pisos · 2 SUM independientes',
    units: 'Unidades de 2 y 3 ambientes',
  ...
  },
  {
    ...
    specs: '5 pisos · Cocheras en planta baja',
